...
import { ensureKeywordForReview, toggleKeywordManualError } from '@seo-addon/utils/keywordReviewApi.js';
import {
    ctaDisplayLabel,
    formatCtaHref,
    isCtaItemInsertable,
    ...
    return (
        <ul className="wp-article-links-keywords">
            {items.map((item, index) => {
                ...
                const count = variant === 'domain-link'
                    ? Number(item?.occurrence_count ?? 0)
                    : Number(item?.article_count ?? 0);
                const countSuffix =
                    variant === 'domain-link' && Number.isFinite(count)
                        ? ` (${count})`
                        : '';
                ...
                const canLocate = variant !== 'domain-link' || count > 0;
                ...
                return (
                    ...
                    >
                        <button
                            ...
                            disabled={variant === 'domain-link' && !canLocate}
                            onMouseDown={(e) => e.preventDefault()}
                            onClick={() => {
                                if (variant === 'domain-link' && !canLocate) {
                                    return;
                                }
                    ...
                    return (
                        ...
                        >
                            ...
                            {showReviewActions ? (
                                <button
                                    type="button"
                                    className={`wp-article-links-review-btn is-danger${isErrorOn ? ' is-on' : ''}`}
                                    aria-label={t('keyword_review_error_button_label', { label })}
                                    title={t('keyword_review_error_button_title')}
                                    aria-pressed={isErrorOn}
                                    disabled={isReviewLoading || isEditing}
                                    onMouseDown={(e) => e.preventDefault()}
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        onToggleError(item, index, itemKey);
                                    }}
                                >
                                    {isReviewLoading ? (
...
function InternalLinksSection({
    ...
    return (
        <div className="wp-article-links-group">
            ...
            {showGenerate ? (
                <div className="wp-article-links-generate-row">
                    <button
                        type="button"
...
export default function ArticleLinksSidebar({
    ...
    const [errorKeywordIds, setErrorKeywordIds] = useState(() => new Set());
    ...
    const applySuggestionPayload = (payload, source, options = {}) => {
        ...
        const incomingInternal = mergeSuggestionCatalog(
            payload.suggestedInternalLinksCatalog ?? [],
            payload.suggestedInternalLinks ?? [],
        );
        const incomingExternal = mergeSuggestionCatalog(
    ...
    const loadLinkSuggestions = async (options = {}) => {
        ...
        try {
            ...
            if (findMore) {
                bumpSuggestionCursor({
                    phase: suggestionCursorRef.current.hasResults ? 'source1_done' : 'idle',
                    hasResults: suggestionCursorRef.current.hasResults,
                });
            }
        } finally {
    ...
    /** @type {React.MutableRefObject<Array<{ id?: string, content?: string, type?: string }>>} */
    ...
    const togglePhraseError = async (item, itemKey) => {
        const text = String(item?.text ?? '').trim();
        if (text === '') {
            return;
        }
        ...
        setReviewLoadingKey(itemKey);
        try {
            const result = await toggleKeywordManualError(keywordId, {
                article_id: articleMetaRef.current.articleId,
                source: 'article_suggestion',
            });
            setErrorKeywordIds((prev) => {
                const next = new Set(prev);
                if (result.manual_error) {
                    next.add(keywordId);
                } else {
            ...
            window.dispatchEvent(
                new CustomEvent('seo-article-editor-notify', {
                    detail: {
                        title: t('keyword_review_error_failed_title'),
                        body: String(error?.message ?? t('keyword_review_error_failed_body')),
                        status: 'danger',
                    },
                }),
            );
        ...
        const onLinksUpdate = (event) => {
            ...
            if (detail.source === 'links-base' || detail.source === 'links-suggestions' || detail.source === 'links-suggestions-fallback') {
                ...
                if (
                    ...
                    if (Array.isArray(detail.editor_blocks) && detail.editor_blocks.length > 0) {
                        editorBlocksRef.current = detail.editor_blocks;
                    }
                    setDomainLinks(
                        applyDomainLinkFilters(
                            allDomainLinksRef.current,
            ...
            if (allDomainLinksRef.current.length > 0) {
                setDomainLinksCollapsed(false);
            }
    ...
    const scrollToDomainItem = (item, itemKey, variant) => {
        ...
        const occurrences = Array.isArray(item?._domain_occurrences)
            ? item._domain_occurrences
            : [];
        ...
        selectedDomainOccurrenceRef.current = {
            itemKey,
            occurrence,
        ...
        if (occurrence) {
            scrollToDomainLinkOccurrence(occurrence, {
                fallbackText: String(item?.text ?? '').trim(),
            });
        } else {
            // Fallback: exact phrase locate via editor expand+scroll path.
            const fallback = String(item?.text ?? '').trim();
            if (fallback !== '') {
                scrollToDomainLinkOccurrence(
                    { matchedText: fallback, blockId: '', matchIndex: 0 },
            ...
            <div className="seo-link-assistant">
                ...
                >
                    <InternalLinksSection
                        ...
                        onSuggestionClick={handleInternalSuggestionClick}
