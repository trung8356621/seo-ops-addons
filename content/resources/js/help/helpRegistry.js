/**
 * Central Help registry — 2-level groups → topics.
 * Modal consumes this; do not hard-code full help copy inside UI components.
 *
 * @typedef {{ title?: string, duration?: string, thumbnail?: string|null, url?: string, longUrl?: string|null }} HelpVideo
 * @typedef {{ type: 'widget'|'module'|'scroll', id: string }} HelpTarget
 * @typedef {{
 *   id: string,
 *   title: string,
 *   summary?: string,
        ...
        topics: [
            ...
            },
            {
                ...
                steps: [
                    ...
                    'Click vào site hoặc project để đi sâu (nếu có link).',
                ],
            },
        ],
    },
        ...
        topics: [
            ...
            },
            {
                ...
                steps: [
                    ...
                    'Theo dõi Queue nếu sync nền kéo dài.',
                ],
            },
        ],
    },
        ...
        topics: [
            {
                ...
                steps: [
                    ...
                    'Watermark / Image editor / Magic Eraser là tool riêng trong Media.',
                ],
            },
            {
                id: 'media-editor',
                ...
                steps: [
                    ...
                    'Kiểm tra kích thước / alt trước khi Sync WP.',
                ],
            },
        ],
    },
        ...
        topics: [
            {
                ...
                steps: [
                    ...
                    'Dùng Performance Hub để xem GSC / rank (nếu đã kết nối).',
                ],
            },
            {
                id: 'articles-optimal',
                ...
                steps: [
                    ...
                    'Ưu tiên bài điểm thấp / violation nhiều.',
        ...
        topics: [
            {
                ...
                steps: [
                    ...
                    'Workflows: prompt mặc định cho create / rewrite / FAQ…',
                ],
            },
            {
                id: 'settings-domain',
...
/**
 * Context keys map route → preferred group + modal title scope.
 * @type {Record<string, HelpContext>}
 */
export const helpRegistry = {
    dashboard: {
        ...
        routeNames: ['filament.seo.pages.dashboard', 'filament.seo.pages.dashboard.connection'],
    ...
    syncQueue: {
        ...
        routeNames: [
            'filament.seo.resources.articles.sync-queue',
            'filament.seo.resources.articles.index',
        ],
    ...
    media: {
        ...
        routeNames: [
            ...
            'filament.seo.pages.watermark-settings',
            'filament.seo.pages.image-processing',
            'filament.seo.pages.image-optimization-settings',
        ],
        pathPatterns: [/\/media(?:\/|$)/, /watermark/, /image-processing/, /image-optimization/],
    ...
    seo: {
        ...
        pathPatterns: [/\/keywords/, /performance-hub/, /articles-optimal/, /ai-keyword/],
    ...
    settings: {
        ...
        routeNames: [
            ...
            'filament.seo-main.resources.prompts.*',
            'filament.admin.resources.prompts.*',
            'filament.admin.resources.tasks.*',
            'filament.seo-main.resources.tasks.*',
            'filament.seo.resources.tasks.*',
            'filament.admin.pages.settings*',
            'filament.seo.pages.seo-team',
            'filament.seo.pages.automation*',
        ],
        pathPatterns: [/\/settings/, /\/domains/, /\/prompts/, /\/admin\/prompts/, /\/admin\/tasks/, /\/seo\/tasks/, /\/seo-team/, /\/automation/],
 ...
 * @returns {HelpGroup|null}
 */
export function findHelpGroup(groupId) {
    const id = String(groupId ?? '').trim();
    if (id === '') {
        return null;
    }

    return HELP_GROUPS.find((group) => group.id === id) ?? null;
}
 ...
 */
export function findHelpTopic(groupId, topicId) {
    const group = findHelpGroup(groupId);
    if (!group) {
        return null;
    }
    ...
    return group.topics.find((topic) => topic.id === id) ?? null;
}

/**
 * @param {HelpContext|null|undefined} context
 * @returns {HelpGroup[]}
 */
export function groupsForContext(context) {
    const ids = Array.isArray(context?.groupIds) && context.groupIds.length > 0
        ? context.groupIds
        : helpRegistry.system.groupIds;

    /** @type {HelpGroup[]} */
    const groups = [];
    for (const id of ids) {
        const group = findHelpGroup(id);
        if (group) {
            groups.push(group);
        }
    }

    return groups.length > 0 ? groups : HELP_GROUPS;
}
