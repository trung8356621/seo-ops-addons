...
final class RunMetadataDomainSyncJob implements ShouldBeUnique, ShouldQueue
{
    use Dispatchable;
    ...
    {
        return 'seo-meta-sync:'.$this->siteId.':'.$this->userId;
    }

    public function handle(
        MetadataDomainSyncRunner $runner,
    ...
    {
        $cacheKey = MetadataDomainSyncCache::cacheKey($this->userId, $this->siteId);
        ...
        if (($state['status'] ?? '') === MetadataDomainSyncCache::STATUS_COMPLETED) {
            return;
        }
