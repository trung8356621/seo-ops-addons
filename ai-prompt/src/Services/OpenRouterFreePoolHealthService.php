...
final class OpenRouterFreePoolHealthService
{
    ...
    public function state(ApiConnection $connection): FreePoolHealthState
    {
        ...
        $this->maybeTransitionWaitingProbe($connection, $snap);

        return FreePoolHealthState::tryFrom((string) ($snap['free_pool_state'] ?? ''))
    ...
    public function skipReasonForCandidate(RoutedAiCandidate $candidate): ?string
    {
        ...
        $isOpenRouter = (string) $connection->provider
            === \Omnichannel\Addons\AiPrompt\Support\ApiConnectionProviders::OPENROUTER;
        ...
        if ($isOpenRouter) {
            ...
            $skip = match ($state) {
                ...
                // WaitingProbe: allow exactly one attempt (list filtered elsewhere); do not skip here.
                FreePoolHealthState::WaitingProbe => null,
    ...
    /**
     * Drop hard-locked / quota / resyncing pools; keep at most one probe candidate per WaitingProbe connection.
     *
    ...
    public function filterCandidatesForCircuit(array $candidates): array
    {
        ...
        foreach ($candidates as $candidate) {
            ...
            $isOpenRouter = (string) $connection->provider
                === \Omnichannel\Addons\AiPrompt\Support\ApiConnectionProviders::OPENROUTER;
            if (! $isOpenRouter) {
                $skip = $this->modelCooldownSkip($candidate);
                if ($skip === null) {
                    $out[] = $candidate;
                }
    ...
    public function recordQualifyingFailure(
        ...
        } elseif ($allFail && $eligible > 0 && $failedDistinct >= $eligible && $failedDistinct >= 1) {
            $shouldHardLock = true;
    ...
    /**
     * Machine-readable alert payload for UI / Content Project.
     *
    ...
    private function maybeEnqueueCatalogResync(
        ...
        try {
            if (function_exists('app') && app()->bound(AiModelRouterService::class)) {
                $sync = app(AiModelRouterService::class);
                $sync->syncModelsForConnection((int) $connection->id);
                $snap['last_catalog_sync_at'] = Carbon::now()->toIso8601String();
     ...
     * @param  array<string, mixed>  $snap
     */
    private function maybeTransitionWaitingProbe(ApiConnection $connection, array &$snap): void
    {
        $state = FreePoolHealthState::tryFrom((string) ($snap['free_pool_state'] ?? ''))
            ?? FreePoolHealthState::Healthy;
        if ($state !== FreePoolHealthState::HardLocked && $state !== FreePoolHealthState::DailyQuotaLocked) {
            return;
        }
        ...
        if ($state === FreePoolHealthState::DailyQuotaLocked) {
            // Daily quota unlock → healthy; next free attempt re-validates.
            $snap['free_pool_state'] = FreePoolHealthState::Healthy->value;
            $snap['free_pool_lock_reason'] = null;
            $snap['free_pool_lock_until'] = null;
            $this->persist($connection, $snap);
    ...
    private function countsTowardPoolRatio(AiFailureDecision $decision): bool
    {
        ...
        // Never count validation/refusal/quality as pool degradation.
    ...
    private function persist(ApiConnection $connection, array $snap): void
    {
        ...
        try {
            // Metadata-only write — avoid dirty columns missing from slim test schemas.
            ApiConnection::query()->whereKey((int) $connection->id)->update([
                'metadata' => $meta,
                'updated_at' => now(),
            ]);
            $connection->syncOriginalAttribute('metadata');
        } catch (\Throwable) {
            // Best-effort; callers still hold in-memory metadata.
        }
    }
    ...
    private function userMessage(ApiConnection $connection, array $snap, FreePoolHealthState $state): string
    {
        ...
        return match ($state) {
            FreePoolHealthState::HardLocked => sprintf(
                '%s Free đang tạm khóa — %d/%d model miễn phí gặp lỗi trong thời gian ngắn. Hệ thống đã dừng thử Free để tránh retry lặp lại.',
                $name,
                $failed,
                max($eligible, $failed),
            ),
            ...
            FreePoolHealthState::WaitingProbe => sprintf(
                '%s Free đang chờ probe trước khi mở lại.',
                $name,
            ),
